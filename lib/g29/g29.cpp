...
void G29Pedals::begin() {
    changed = false;
}

void G29Pedals::end() {
    HID().SendReport(REPORT_ID, &report, sizeof(g29pedals_report_struct));
}

void G29Pedals::setAccelerator(uint16_t v) {
    report.accelerator = v;
    changed = true;
}

void G29Pedals::setBrake(uint16_t v) {
    report.brake = v;
    changed = true;
}

void G29Pedals::setClutch(uint16_t v) {
    report.clutch = v;
    changed = true;
}
